 ...
 */
 	...
 	var $startRec;
	/**
		* Gets the number of record in the recordset for the given model
	*/
	
	var $endRec;
	/**
		* Gets the end number of record in the recordset for the given model
	*/
	
	var $special_case	=	array('total_quantity','min_price','NEW','USED','discounted_price');
 ...
 */
	function sortKey($model = null, $options = array()) {
		...
		if(!is_null($params['defaults']['passit']) && in_array($params['defaults']['passit']['sort'],$this->special_case))
		{
			
			$options['order'][$params['defaults']['passit']['sort']]	=	($params['options']['passit']['direction']=="asc") ? "desc":"asc";
		}
 ...
 */
	function sortDir($model = null, $options = array()) {
		...
		if(!is_null($params['defaults']['passit']) && in_array($params['defaults']['passit']['sort'],$this->special_case))
		{
			$options['order'][$params['defaults']['passit']['sort']]	=	$params['options']['passit']['direction'];
		}
 ...
 */
	function sort($title, $key = null, $options = array()) {
		...
		$order_default	=	explode(" ",$this->params['paging'][$this->params['models'][0]]['defaults']['order'][0]);
		
		
		if ($this->sortKey($options['model']) == $key)
		{
			if($this->sortKey($options['model'])!==0 or ($order_default[0]==$key))
			{
				$options['class']	=	$current;
				if($dir=='asc')
				{
					$title	.=	" <img src='{$this->webroot}img/sort_up.png' border=0>";
				}
				else
				{
					$title	.=	" <img src='{$this->webroot}img/sort_down.png' border=0>";
				}
			}
		}
 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		if(!is_null($params['defaults']['passit']) && in_array($paging['defaults']['passit']['sort'],$this->special_case))
		{
			
			$url = array_merge(array('page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step),'sort'=>$paging['defaults']['passit']['sort'],'direction'=>$paging['options']['passit']['direction']), $url);
 ...
 */
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, $params['page'], array('class' => $options['current']));
			...
			for ($i = $start; $i < $end; $i++) {
				if(!is_null($params['defaults']['passit']) && in_array($params['defaults']['passit']['sort'],$this->special_case))
				{
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i,'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options)). $separator;
				}
				else
				{
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options)). $separator;
			...
			if ($end != $params['page']) {
				if(!is_null($params['defaults']['passit']) && in_array($params['defaults']['passit']['sort'],$this->special_case))
				{
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $end,'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options));
				}
				else
				{
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $i, array('class' => $options['current']));
				} else {
					if(!is_null($params['defaults']['passit']) && in_array($params['defaults']['passit']['sort'],$this->special_case))
					{
						$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i,'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options));
					}
					else
					{
 ...
 */
	function first($first = '<< first', $options = array()) {
		...
		if (is_int($first) && $params['page'] > $first) {
			...
			if(!is_null($params['defaults']['passit']) && in_array($paging['defaults']['passit']['sort'],$this->special_case))
			{
				$out .= $this->Html->tag($tag, $this->link($first, array('page' => 1,'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options)) . $after;
			}
			else
			{
				$out = $this->Html->tag($tag, $this->link($first, array('page' => 1,'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options)) . $after;
 ...
 */
	function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] < $lower) {
			...
			if(!is_null($params['defaults']['passit']) && in_array($paging['defaults']['passit']['sort'],$this->special_case))
			{
				$out .= $this->Html->tag($tag, $this->link($last, array('page' => $params['pageCount'],'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options));
			}
			else
			{
				$out = $before . $this->Html->tag($tag, $this->link($last, array('page' => $params['pageCount'],'sort'=>$params['defaults']['passit']['sort'],'direction'=>$params['options']['passit']['direction']), $options));
			}
		}
		return $out;
	}
