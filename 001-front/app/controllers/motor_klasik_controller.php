<?php
class MotorKlasikController extends AppController
{
	var $components = array('Cookie');
	var $name		=	"MotorKlasik";
	var $uses		=	null;
	...
	function Index($category_id="all_categories",$current_city="all_cities")
	{
		...
		$filtering			=	$this->Product->GetData($category_id,$current_city,"MotorKlasik");
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'			=>	20,
		...
		if(empty($data))
		{
			...
			$category_others	=	$this->Product->GetCategoryOthers($category_id,$current_city,$type="MotorKlasik");
			$this->set(compact("category_others"));
			
			$province_others	=	$this->Product->GetProvinceOthers($category_id,$current_city,$type="MotorKlasik");
