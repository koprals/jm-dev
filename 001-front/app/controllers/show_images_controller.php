...
class ShowImagesController extends AppController
{
	...
	function RefreshImages()
	{
		...
		$data		=	$this->ProductImage->find('all',array('fields'=>array('ProductImage.id')));
		
		
		foreach($data as $data)
		{
			$file_path	= $settings['Setting']['path_content']."ProductImage/".$data['ProductImage']['id'];
			...
			while (($sf = readdir($dh)) !== false)
			{
				...
				$filepath	=	(substr($file_path, -1) != "/")? $file_path."/":$file_path;
				$info		=	pathinfo($filepath.$sf,PATHINFO_FILENAME);
				if($info!==$data['ProductImage']['id'])
				{
					@unlink($filepath.$sf);
				}
			}
	...
	function Index()
	{
		...
		$img->watermarkimg	=	$settings['Setting']['path_webroot']."img/logo_baru.jpg";
