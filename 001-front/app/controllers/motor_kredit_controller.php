<?php
class MotorKreditController extends AppController
{
	var $components = array('Cookie');
	var $name		=	"MotorKredit";
	var $uses		=	null;
	...
	{
		//DEFINE DATA TO VIEW
		$this->set("current_menu","motor_kredit");
		...
		$filtering			=	$this->Product->GetData($category_id,$current_city,"MotorKredit");
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'			=>	20,
		...
		if(empty($data))
		{
			...
			$category_others	=	$this->Product->GetCategoryOthers($category_id,$current_city,$type="MotorKredit");
			$this->set(compact("category_others"));
			
			$province_others	=	$this->Product->GetProvinceOthers($category_id,$current_city,$type="MotorKredit");
