...
<?php echo $this->requestAction("/Template/DaftarKota/{$current_category_id}/{$current_city}/DaftarMotor",array("return"))?>
...
function GotoPage(value)
{
	...
	location.href	=	'<?php echo $settings['site_url']."DaftarMotor/".implode("/",$this->params['pass'])?>/page:'+val;
}
    ...
    <div class="<?php echo $product_hot?>" <?php echo $style;?> >
    	...
    	<div class="gambar" style="height:105px;border:0px solid black; margin-top:5px;">
        	<div class="kiri right8" style="border:0px solid black;width:110px;">
            	<a href="<?php echo $settings['site_url']?>Iklan/Detail/<?php echo $data["Product"]["id"]."/".$general->seoUrl("motor dijual ".$data["Parent"]["name"]." ".$data["Category"]["name"]." (".$data["Product"]["thn_pembuatan"].") ".$data["ProvinceGroup"]["name"]).".html"?>" link="<?php echo $settings['site_url']?>Template/HoverImg/<?php echo $data['Product']['id']?>" rel="img_thumb" id="hover_<?php echo $data['Product']['id']?>">
					<img src="<?php echo $settings['showimages_url']."?code=".$data['ProductImage']['id']."&prefix=_110_100&content=ProductImage&w=110&h=100"?>" border="0" alt="<?php echo $data["Parent"]["name"]." ".$data["Category"]["name"]." (".$data["Product"]["thn_pembuatan"].") ".$data["ProvinceGroup"]["name"]?>"/>
				</a>
            </div>
