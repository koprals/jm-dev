    ...
    function generate ($data, $settings = array ()) {
        ...
        foreach ($data as $i => $result) {
            ...
            if ($element) {
				...
				if(!empty($result[$model]['url']))
				{
					if( substr($result[$model]['url'],0,4)=="http" )
					{
						$valid_url	=	$result[$model]['url'];
					}
					else
					{
						$valid_url	=	$ROOT."/".$result[$model]['url'];
					}
				}
				
				
				$parent_name	=	($result['Parent']['name']!=="TOP") ? $result['Parent']['name']." " : "";
				
				$GetSeoName		=	($current_city!="all_cities") ? implode("_",array($prefix_seo,$this->GetSeoName($parent_name.$result[$model]['name']),$this->GetSeoName($city_name).".html")) :  implode("_",array($prefix_seo,$this->GetSeoName($parent_name.$result[$model]['name']).".html"));
				
				$url			=	empty($result[$model]['url']) ? $link.$result[$model]['id']."/".$current_city."/".$GetSeoName : $valid_url;
				$target			=	$result[$model]['target'];
				$alias_truncate	=	substr($result[$model][$alias],0,17);
				$arrow		=	($hasVisibleChildren) ? '<span class="arrowdiv">&nbsp;&nbsp;</span>' : '';
				
				if($current_id	==	$result[$model]['id'] or $current_parent_id ==	$result[$model]['id'])
				{
					$content 	= '<a href="'.$url.'" class="current" title="'.$result[$model][$alias].'">'.$alias_truncate.$arrow.'</a>';
				}
				else
				{
					$content = "<a href='".$url."' target='".$target."' title='".$result[$model][$alias]."'>".$alias_truncate."</a>";
				}
            ...
            if ($itemType) {
				...
				if($lastChild)
				{
					$return .= '<' . $itemType . $itemAttributes . ' style="padding-bottom:10px;">';	
				}
				else
				{
