...
{
	$("#UserIndexForm").ajaxSubmit({
		url			: "<?php echo $settings['site_url']?>CpanelPhoto/ProcessUpload",
		type		: "POST",
		dataType	: "json",
	...
	$("#UserIndexForm").ajaxSubmit({
		...
		success		: function(data)
		{
			...
			else
			{
				...
				$("#UserPhoto").attr("src","<?php echo $settings['showimages_url']?>?code=<?php echo $profile['User']['id']?>&prefix=_prevthumb&content=User&w=120&h=120&time"+(new Date()).getTime());
				$("#cancelButton").fadeIn(300);
				$("#LoadingPhoto").hide();
				$("#UserPhoto").show(300);
				$("#error").find("div[id=error_list]").html(data.error);
...
<div class="line">
    <div class="size100 tengah">
		<div class="text_title3">
            <div class="line1">Upload My Photo</div>
        </div>
        <div class="line back3 size100 kiri position1 rounded2" style="padding-bottom:10px;">
        	...
        	<?php echo $form->create("User",array("type"=>"file","onsubmit"=>"return SubmitUpdate()"))?>
            <div class="tengah size60" style="border:0px solid black;">
            	<div class="kiri top50" style="width:120px; height:120px;border:0px solid white;">
                    <div class="kiri" style="width:120px;height:auto;">
                        ...
                        <img src="<?php echo $settings['showimages_url']?>?code=<?php echo $profile['User']['id']?>&prefix=_prevthumb&content=User&w=120&h=120" id="UserPhoto" style="border:1px solid #ffffff; padding:2px;"/>
