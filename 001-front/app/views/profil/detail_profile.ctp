...
{
	$data['Profile']['address']	=	str_replace(chr(10),' ',$data['Profile']['address']);
}
if(!empty($profile['Company']['address']))
{
	$data['Company']['address']	=	str_replace(chr(10),' ',$data['Company']['address']);
}
?>
<script>
$(document).ready(function(){
	$("#list_item").load("<?php echo $settings['site_url']?>Profil/ListItemProfile/<?php echo $data["Profile"]["user_id"]?>");
});
            ...
            <div class="kiri size100 top10">
                <div class="kiri size100" style="border-bottom:1px solid #cccccc; padding-bottom:10px; padding-top:10px;">
                    ...
                    <div class="kiri size69">
                    	<div class="kiri">
                    		<a href="javascript:void(0)" style="border:none; outline:none" onClick="$.prettyPhoto.open('<?php echo $settings['showimages_url']?>?code=<?php echo $data['User']['id']?>&prefix=_1000_600&content=User&w=1000&h=600&nopict=noimages');"><img src="<?php echo $settings['showimages_url']?>?code=<?php echo $data["User"]["id"]?>&prefix=_121_84&content=User&w=121&h=84" style="border:1px solid #cccccc; padding:2px;"/></a>
                        </div>
                        <div class="kiri left10 size60">
                        	<span class="kiri size100 style1 red2 normal text16 bold"> <?php echo $data["Profile"]["fullname"]?></span>
                ...
                <div class="kiri size100" style="border-bottom:1px solid #cccccc; padding-bottom:10px; padding-top:10px;">
                    ...
                    <div class="kiri size69 style1 black2 text12">
                    	<?php echo $data["Profile"]["phone"].$extend_phone?>
                    </div>
                </div>
                ...
                <?php if($data["Profile"]["user_id"] == $profile["User"]["id"]):?>
                <div class="kiri size100 top10">
                	<input type="button" name="button" value="EDIT PROFIL" class="tombol1" onclick="location.href='<?php echo $settings['site_url']?>Cpanel/UpdateProfile'" style="float:left"/>
                </div>
                <?php endif;?>
            </div>
        </div>
...
<!--- MAPS -->
<?php if(!empty($data["Profile"]["lat"]) && !empty($data["Profile"]["lng"])):?>
...
var center 		=	new GLatLng('<?php echo $data["Profile"]["lat"]?>','<?php echo $data["Profile"]["lng"]?>');
...
var w_marker 				=	new GMarker(new GLatLng('<?php echo $data["Profile"]["lat"]?>','<?php echo $data["Profile"]["lng"]?>'),{clickable:true, autoPan:false,icon:letteredIcon});
...
function out()
{
	...
	<div class="kiri size100" style="height:90px;border:0px solid black;">\
		<div class="kiri left5" style="width:80px; border:0px solid black;">\
			<img src="<?php echo $settings['showimages_url']."?code=".$data["User"]["id"]?>&prefix=_70_70&content=User&w=70&h=70" style="border:1px solid #cccccc">\
		</div>\
		<div class="kiri" style="width:145px; border:0px solid black;">\
			<div class="kiri size100"><a href="" class="style1 text12 black1 bold normal"><?php echo $data["Profile"]["fullname"]?></a></div>\
			<div class="kiri size100 style1 text11 black1 normal"><?php echo strip_tags(htmlentities($data["Profile"]["address"],ENT_QUOTES))?></div>\
...
function SubmitPm()
{
	$("#SendPmForm").ajaxSubmit({
		url			: "<?php echo $settings['site_url']?>Profil/SendMessageProfile",
		type		: "POST",
		dataType	: "json",
		clearForm	: false,
		beforeSend	: function()
...
<div class="line">
    <div class="size100 tengah">
        ...
        <div class="line size100 kiri position1" style="padding-bottom:10px;" id="updt_prfl">
            ...
            <?php echo $form->create("Profile",array("onsubmit"=>"return SubmitPm()","id"=>"SendPmForm"))?>
             <?php echo $form->input("profile_id",array("type"=>"hidden","readonly"=>true,"value"=>$data["Profile"]["id"]))?>
            <div class="kiri size95 left10" style="border:0px solid black;">
                <div class="line top20">
                    ...
                    <div class="kiri style1 text14 bold white" style="border:0px solid black">
                    	<img src="<?php echo $settings['showimages_url']?>?code=<?php echo $data["User"]["id"]?>&prefix=_50_50&content=User&w=50&h=50" id="UserPhoto" style="border:1px solid #cccccc; padding:2px;"/>
                    </div>
                     <div class="kiri style1 text14 bold black1 left5" style="border:0px solid black">
                    	<?php echo $data['Profile']["fullname"]?><br>
                ...
                <div class="kiri size100 top15">
                    ...
                    <div class="kiri size69">
                        <?php echo $form->input("subject",array("div"=>false,"label"=>false,"type"=>"text","title"=>"Berikan subjek untuk pesan anda.","class"=>"input7 style1 black1 text12 size80 kiri" ,"value"=>"Saya tertarik dengan motor yang anda jual di ".$settings['site_name']."."))?>
...
<div class="size100 tengah" style="border:0px solid black;">
    <div class="text_title3">
        <div class="line1">Detail member tidak ditemukan.</div>
    </div>
    <div class="line size100 kiri position1 rounded2" style="padding-bottom:10px; background-color:#888888; background-image:none;">
        ...
        <div class="kiri size65 left20 style1 white text12 top10 bold">
        	Maaf detail member yang anda cari tidak kami temukan.<br /><br />
            Mungkin member yang anda cari telah dihapus oleh admin kami.<br /><br />
        </div>
    </div>
    <div class="line">&nbsp;</div>
</div>
