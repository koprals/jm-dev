<?php
class ApproveProductController extends AppController
{
	var $name		=	"ApproveProduct";
	...
	function beforeFilter()
	{
		...
		$this->set('child_code','approve_product');
		$this->layout	=	"new";
	}
	
	function Index()
	{
		$this->Session->delete('CondSearch.ApproveProduct');
	...
	function ListItem()
	{
		...
		if(!empty($keywords) && !empty($_POST['btn_keywords']))
		{
			...
			$this->Session->write("CondSearch.ApproveProduct",$cond_search);
		...
		$ses_cond			=	$this->Session->read("CondSearch.ApproveProduct");
