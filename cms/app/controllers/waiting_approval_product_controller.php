<?php
class WaitingApprovalProductController extends AppController
{
	var $name		=	"WaitingApprovalProduct";
	...
	function beforeFilter()
	{
		...
		$this->set('child_code','waiting_approval');
		$this->layout	=	"new";
	}
	
	function Index()
	{
		$this->Session->delete('CondSearch.WaitingApproval');
	...
	function ListItem()
	{
		...
		if(!empty($keywords) && !empty($_POST['btn_keywords']))
		{
			...
			$this->Session->write("CondSearch.WaitingApproval",$cond_search);
		...
		$cond_search		=	array();
		$filter_paginate	=	array('Product.productstatus_id' => 0);
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'		=>	$viewpage,
				'order'		=>	$order,
		...
		$ses_cond			=	$this->Session->read("CondSearch.WaitingApproval");
