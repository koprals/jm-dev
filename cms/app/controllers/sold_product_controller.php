<?php
class SoldProductController extends AppController
{
	var $name	=	"SoldProduct";
	...
	function beforeFilter()
	{
		...
		$this->set('child_code','sold_product');
		$this->layout	=	"new";
	}
	
	function Index()
	{
		$this->Session->delete('CondSearch.SoldProduct');
	...
	function ListItem()
	{
		...
		if(!empty($keywords) && !empty($_POST['btn_keywords']))
		{
			...
			$this->Session->write("CondSearch.SoldProduct",$cond_search);
		...
		$cond_search		=	array();
		$filter_paginate	=	array('Product.productstatus_id' => 1,'Product.sold' => 1);
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'		=>	$viewpage,
				'order'		=>	$order,
		...
		$ses_cond			=	$this->Session->read("CondSearch.SoldProduct");
