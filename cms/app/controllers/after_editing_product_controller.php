<?php
class AfterEditingProductController extends AppController
{
	var $name		=	"AfterEditingProduct";
	...
	function beforeFilter()
	{
		...
		$this->set('child_code','after_editing');
		$this->layout	=	"new";
	}
	
	function Index()
	{
		$this->Session->delete('CondSearch.AfterEditing');
	...
	function ListItem()
	{
		...
		if(!empty($keywords) && !empty($_POST['btn_keywords']))
		{
			...
			$this->Session->write("CondSearch.AfterEditing",$cond_search);
		...
		//DEFINE FILTERING
		$cond_search		=	array();
		$filter_paginate	=	array('Product.productstatus_id' => -2);
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'		=>	$viewpage,
				'order'		=>	$order,
		...
		$ses_cond			=	$this->Session->read("CondSearch.AfterEditing");
