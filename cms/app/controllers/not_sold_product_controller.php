<?php
class NotSoldProductController extends AppController
{
	var $name		=	"NotSoldProduct";
	...
	function beforeFilter()
	{
		...
		$this->set('child_code','notsold_product');
		$this->layout	=	"new";
	}
	
	function Index()
	{
		$this->Session->delete('CondSearch.NotSoldProduct');
	...
	function ListItem()
	{
		...
		if(!empty($keywords) && !empty($_POST['btn_keywords']))
		{
			...
			$this->Session->write("CondSearch.NotSoldProduct",$cond_search);
		...
		$cond_search		=	array();
		$filter_paginate	=	array('Product.productstatus_id' => 1,'Product.sold' => 0);
		$this->paginate		=	array(
			'Product'	=>	array(
				'limit'		=>	$viewpage,
				'order'		=>	$order,
		...
		$ses_cond			=	$this->Session->read("CondSearch.NotSoldProduct");
