...
function select_all(){
	...
	$.getJSON('<?php echo $settings['cms_url']?>Product/SelectAll/-1/null/CondSearch.EditingRequired',function(data){
	...
	if(confirm_delete==true)
	{
		$("#SearchListItemForm").ajaxSubmit({
			...
			success:function(data)
			{
				...
				$("#list_item").load("<?php echo $settings['cms_url']?>EditingRequiredProduct/ListItem/page:<?php echo $page?>/limit:<?php echo $viewpage.$ordered?>");
				alert(data.messages);
			}
		});
	}
...
<?php echo $paginator->options(array(
	'url'	=> array(
		'controller'	=> 'EditingRequiredProduct',
		'action'		=> 'ListItem'
	),
    ...
    <div class="left" style="width:60%;margin-top:8px;">
        <div class="text7" style="width:100%; float:left">
            ...
            <?php if($paginator->hasPrev()):?>
                <a href="<?php echo $settings['cms_url']?>EditingRequiredProduct/ListItem/page:<?php echo ($page-1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_left" onclick="return onClickPage(this,'#list_item')"></a>
            ...
            <?php if($paginator->hasNext()):?>
                <a href="<?php echo $settings['cms_url']?>EditingRequiredProduct/ListItem/page:<?php echo ($page+1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_right" onclick="return onClickPage(this,'#list_item')"></a>
            <?php endif;?>
            <span style="float:left;"><?php echo $paginator->counter(array('format' => 'of %pages% pages'));?> | View </span>
            
            <?PHP echo $form->select("view",array(1=>1,5=>5,10=>10,20=>20,100=>100,200=>200,1000=>1000),$viewpage,array("class"=>"text7","style"=>"width:auto; height:22px; float:left; margin:-5px 3px 0 3px; border:1px solid #c8c8c8; cursor:pointer;","onchange"=>"onClickPage('".$settings['cms_url']."EditingRequiredProduct/ListItem/limit:'+this.value+'".$ordered."','#list_item')","empty"=>false))?>
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color:#dadfe0; border-collapse:collapse;" onmouseout="OnMouseOutTable()" id="table" class="table_off">
        ...
        <tr bgcolor="<?php echo $back?>" onMouseOver="OnMouseOverTr('<?php echo $data['Product']['id']?>')" id="tr_<?php echo $data['Product']['id']?>">
            ...
            <td style="border:1px solid #D3C8AA; text-align:center;">
            	<?php if($data['Product']['productstatus_user']==1):?>
                <div class="line2">
                    <a href="<?php echo $settings['cms_url']?>Product/Edit/<?php echo $data['Product']['id']?>/editing_required" class="nav_6">edit</a>
