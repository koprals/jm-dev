...
function select_all(){
	...
	$.getJSON('<?php echo $settings['cms_url']?>Product/SelectAll/1/0/null/CondSearch.NotSoldProduct',function(data){
	...
	if(confirm_delete==true)
	{
		$("#SearchListItemForm").ajaxSubmit({
			...
			success:function(data)
			{
				...
				$("#list_item").load("<?php echo $settings['cms_url']?>NotSoldProduct/ListItem/page:<?php echo $page?>/limit:<?php echo $viewpage.$ordered?>");
				alert(data.messages);
			}
		});
	}
...
function Action(val)
{
	...
	else
	{
		$("select#select_action option:selected").each(function(){
			...
			else if($(this).val()=="NotSold")
			{
				ChangeDataConfirmation("all",'diupdate','mengupdate','ChangeToNotSold');
			}
			else if($(this).val()=="Sold")
			{
				ChangeDataConfirmation("all",'diupdate','mengupdate','ChangeToSold');
			}
			else
			{
    ...
    <div class="left" style="width:60%;margin-top:8px;">
        <div class="text7" style="width:100%; float:left">
            ...
            <?php if($paginator->hasPrev()):?>
                <a href="<?php echo $settings['cms_url']?>NotSoldProduct/ListItem/page:<?php echo ($page-1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_left" onclick="return onClickPage(this,'#list_item')"></a>
            ...
            <?php if($paginator->hasNext()):?>
                <a href="<?php echo $settings['cms_url']?>NotSoldProduct/ListItem/page:<?php echo ($page+1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_right" onclick="return onClickPage(this,'#list_item')"></a>
            <?php endif;?>
            <span style="float:left;"><?php echo $paginator->counter(array('format' => 'of %pages% pages'));?> | View </span>
            
            <?PHP echo $form->select("view",array(1=>1,5=>5,10=>10,20=>20,100=>100,200=>200,1000=>1000),$viewpage,array("class"=>"text7","style"=>"width:auto; height:22px; float:left; margin:-5px 3px 0 3px; border:1px solid #c8c8c8; cursor:pointer;","onchange"=>"onClickPage('".$settings['cms_url']."NotSoldProduct/ListItem/limit:'+this.value+'".$ordered."','#list_item')","empty"=>false))?>
  ...
  <div class="table1">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color:#dadfe0; border-collapse:collapse;" onmouseout="OnMouseOutTable()" id="table" class="table_off">
        <tr class="table2" height="30">
            ...
            <td class="table_text1" style="border:1px solid #D3C8AA;width:120px;"><?php echo $paginator->sort('Modified On', 'Product.modified',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td class="table_text1" style="border:1px solid #D3C8AA;width:120px;"><?php echo $paginator->sort('Modified By', 'Product.modified_by',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
        ...
        <tr bgcolor="<?php echo $back?>" onMouseOver="OnMouseOverTr('<?php echo $data['Product']['id']?>')" id="tr_<?php echo $data['Product']['id']?>">
            ...
            <td class="table_text2" style="border:1px solid #D3C8AA;" onClick="OnClickTr('<?php echo $data['Product']['id']?>')"><?php echo date("d-M-Y H:i:s",strtotime($data['Product']['modified']))?></td>
            
            <td class="table_text3" style="border:1px solid #D3C8AA;" onClick="OnClickTr('<?php echo $data['Product']['id']?>')">
				<?php if(!empty($data['Product']['modified_by'])){echo $data['Product']['modified_by'];}else{echo "-";}?>
            </td>
            <td style="border:1px solid #D3C8AA; text-align:center;">
            	<?php if($data['Product']['productstatus_user']==1):?>
                <div class="line2">
                    <a href="<?php echo $settings['cms_url']?>Product/Edit/<?php echo $data['Product']['id']?>/notsold_product" class="nav_6">edit</a>
