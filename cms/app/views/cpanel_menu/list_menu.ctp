...
<?php
$order		=	array_keys($this->params['paging']['CpanelMenu']['options']['order']);
$direction	=	$this->params['paging']['CpanelMenu']['options']['order'];
...
$("#select_all").click(function(){
	...
	$("input[name='data[CpanelMenu][id][]']").each(function(){
		myCars[$(this).val()]	=	$(this).val();
	});
	$("#test_select_all").html('');
	$.getJSON('<?php echo $settings['cms_url']?>CpanelMenu/SelectAll',function(data){
		$.each(data, function(i, item){
			if(!in_array(item.CpanelMenu.id,myCars))
			{
				$("#test_select_all").append('<input name="data[CpanelMenu][id][]" type="hidden" value="'+item.CpanelMenu.id+'" checked="checked">');
			}
		});	
	});
...
function Search()
{
	...
	var toppos		=	pos.left;
	$("#SearchListMenuForm").ajaxSubmit({
		url:'<?php echo $settings['cms_url']?>CpanelMenu/ListMenu',
		type:'POST',
		dataType: "html",
		clearForm:false,
...
{
	OnClickTr(ID);
	var a 	=	confirm('Do you realy want to DELETE menu WHERE id =  '+ID);
	if(a==true)
	{
		DeleteMenu();
	}
...
function DeleteMenu()
{
	...
	$("#SearchListMenuForm").ajaxSubmit({
		url:'<?php echo $settings['cms_url']?>CpanelMenu/Delete',
		type:'POST',
		dataType: "json",
		clearForm:false,
		...
		success:function(data)
		{
			...
			$(".side_center").load("<?php echo $settings['cms_url']?>CpanelMenu/ListMenu/page:<?php echo $page?>/limit:<?php echo $viewpage?>");
			alert(data.messages);
		}
	});
	return false;
...
<?php echo $paginator->options(array(
	'url'	=> array(
		'controller'	=> 'CpanelMenu',
		'action'		=> 'ListMenu'
	),
	'onclick'=>"return onClickPage(this,'.side_center');")
);?>
<div id="output"></div>
...
<div class="line3">
    <a href="<?php echo $settings['cms_url'] ?>CpanelMenu/Add/" class="nav_6" style="float:right;"><img src="<?php echo $this->webroot?>img/icn_add.gif" border="0" style="float:left; margin-right:3px;" />Add Item</a>
</div>
<div class="line1">
    <div class="left" style="width:60%;margin-top:8px;">
        <div class="text7" style="width:100%; float:left">
            ...
            <?php if($paginator->hasPrev()):?>
                <a href="<?php echo $settings['cms_url']?>CpanelMenu/ListMenu/page:<?php echo ($page-1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_left" onclick="return onClickPage(this,'.side_center')"></a>
            ...
            <?php if($paginator->hasNext()):?>
                <a href="<?php echo $settings['cms_url']?>CpanelMenu/ListMenu/page:<?php echo ($page+1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_right" onclick="return onClickPage(this,'.side_center')"></a>
            <?php endif;?>
            <span style="float:left;"><?php echo $paginator->counter(array('format' => 'of %pages% pages'));?> | View </span>
            
            <?PHP echo $form->select("view",array(1=>1,5=>5,10=>10,20=>20,100=>100,200=>200,1000=>1000),$viewpage,array("class"=>"text7","style"=>"width:auto; height:22px; float:left; margin:-5px 3px 0 3px; border:1px solid #c8c8c8; cursor:pointer;","onchange"=>"onClickPage('".$settings['cms_url']."CpanelMenu/ListMenu/limit:'+this.value+'".$ordered."','.side_center')","empty"=>false))?>
  ...
  <div class="table1">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color:#dadfe0; border-collapse:collapse;" onmouseout="OnMouseOutTable()">
        <tr class="table2">
            ...
            <td width="8%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('ID', 'CpanelMenu.id',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="14%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Code', 'CpanelMenu.code',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="19%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Name', 'CpanelMenu.name',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            ...
            <td width="12%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Link', 'CpanelMenu.url',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="10%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Taget Link', 'CpanelMenu.target',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
        ...
        <tr class="table3" style="border:1px solid #D3C8AA;">
            ...
            <td style="border:1px solid #D3C8AA;">
                <?php echo $form->input("code",array("class"=>"all_input2","label"=>false,"div"=>false,"value"=>$code))?>
            </td>
            <td style="border:1px solid #D3C8AA;">
        ...
        <tr bgcolor="<?php echo $back?>"  onClick="OnClickTr('<?php echo $data['CpanelMenu']['id']?>')" onMouseOver="OnMouseOverTr('<?php echo $data['CpanelMenu']['id']?>')" id="tr_<?php echo $data['CpanelMenu']['id']?>">
            <td style="border:1px solid #D3C8AA;">
                <input name="data[CpanelMenu][id][]" type="checkbox" value="<?php echo $data['CpanelMenu']['id']?>" style="margin:5px auto; display:block;" onClick="OnClickTr('<?php echo $data['CpanelMenu']['id']?>')" />
            </td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $data['CpanelMenu']['id']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['CpanelMenu']['code']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['CpanelMenu']['name']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['Parent']['name']?></td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $settings['site_url'].$data['CpanelMenu']['url']?></td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $data['CpanelMenu']['target']?></td>
            <td style="border:1px solid #D3C8AA;">
                <div class="line2">
                    <a href="<?php echo $settings['cms_url']?>CpanelMenu/Add/<?php echo $data['CpanelMenu']['id']?>" class="nav_6">edit</a>
                    <a href="javascript:void(0)" class="nav_6" onclick="DeleteItem('<?php echo $data['CpanelMenu']['id']?>')">delete</a>
                </div>
            </td>
        </tr>
        <?php endforeach;?>
