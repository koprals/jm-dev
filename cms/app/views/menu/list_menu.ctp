...
<?php
$order		=	array_keys($this->params['paging']['CmsMenu']['options']['order']);
$direction	=	$this->params['paging']['CmsMenu']['options']['order'];
...
$("#select_all").click(function(){
	...
	$("input[name='data[CmsMenu][id][]']").each(function(){
		myCars[$(this).val()]	=	$(this).val();
	});
	$("#test_select_all").html('');
	$.getJSON('<?php echo $settings['cms_url']?>Menu/SelectAll',function(data){
		$.each(data, function(i, item){
			if(!in_array(item.CmsMenu.id,myCars))
			{
				$("#test_select_all").append('<input name="data[CmsMenu][id][]" type="hidden" value="'+item.CmsMenu.id+'" checked="checked">');
			}
		});	
	});
...
function Search()
{
	...
	$("#SearchListMenuForm").ajaxSubmit({
		url:'<?php echo $settings['cms_url']?>Menu/ListMenu',
		type:'POST',
		dataType: "html",
		clearForm:false,
...
function DeleteMenu()
{
	...
	$("#SearchListMenuForm").ajaxSubmit({
		url:'<?php echo $settings['cms_url']?>Menu/Delete',
		type:'POST',
		dataType: "json",
		clearForm:false,
		...
		success:function(data)
		{
			...
			$("#list_item").load("<?php echo $settings['cms_url']?>Menu/ListMenu/page:<?php echo $page?>/limit:<?php echo $viewpage.$ordered?>");
			alert(data.messages);
		}
	});
	return false;
...
<div class="line3">
    <a href="<?php echo $settings['cms_url'] ?>Menu/Add/" class="nav_6" style="float:right;"><img src="<?php echo $this->webroot?>img/icn_add.gif" border="0" style="float:left; margin-right:3px;" />Add Item</a>
</div>
<div class="line1">
    <div class="left" style="width:60%;margin-top:8px;">
        <div class="text7" style="width:100%; float:left">
            ...
            <?php if($paginator->hasPrev()):?>
                <a href="<?php echo $settings['cms_url']?>Menu/ListMenu/page:<?php echo ($page-1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_left" onclick="return onClickPage(this,'#list_item')"></a>
            ...
            <?php if($paginator->hasNext()):?>
                <a href="<?php echo $settings['cms_url']?>Menu/ListMenu/page:<?php echo ($page+1)?>/limit:<?php echo $viewpage.$ordered?>" class="nav_table_right" onclick="return onClickPage(this,'#list_item')"></a>
            <?php endif;?>
            <span style="float:left;"><?php echo $paginator->counter(array('format' => 'of %pages% pages'));?> | View </span>
            
            <?PHP echo $form->select("view",array(1=>1,5=>5,10=>10,20=>20,100=>100,200=>200,1000=>1000),$viewpage,array("class"=>"text7","style"=>"width:auto; height:22px; float:left; margin:-5px 3px 0 3px; border:1px solid #c8c8c8; cursor:pointer;","onchange"=>"onClickPage('".$settings['cms_url']."Menu/ListMenu/limit:'+this.value+'".$ordered."','#list_item')","empty"=>false))?>
  ...
  <div class="table1">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-color:#dadfe0; border-collapse:collapse;" onmouseout="OnMouseOutTable()">
        <tr class="table2">
            ...
            <td width="8%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('ID', 'CmsMenu.id',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="14%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Code', 'CmsMenu.code',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="19%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Name', 'CmsMenu.name',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            ...
            <td width="12%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Link', 'CmsMenu.url',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
            <td width="10%" class="table_text1" style="border:1px solid #D3C8AA;"><?php echo $paginator->sort('Taget Link', 'CmsMenu.target',array('class'=>'table_text1','escape'=>false,'current'=>'current_sort'));?></td>
        ...
        <tr bgcolor="<?php echo $back?>"  onClick="OnClickTr('<?php echo $data['CmsMenu']['id']?>')" onMouseOver="OnMouseOverTr('<?php echo $data['CmsMenu']['id']?>')" id="tr_<?php echo $data['CmsMenu']['id']?>">
            <td style="border:1px solid #D3C8AA;">
                <input name="data[CmsMenu][id][]" type="checkbox" value="<?php echo $data['CmsMenu']['id']?>" style="margin:5px auto; display:block;" onClick="OnClickTr('<?php echo $data['CmsMenu']['id']?>')" />
            </td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $data['CmsMenu']['id']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['CmsMenu']['code']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['CmsMenu']['name']?></td>
            <td class="table_text3" style="border:1px solid #D3C8AA;"><?php echo $data['Parent']['name']?></td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $settings['cms_url'].$data['CmsMenu']['url']?></td>
            <td class="table_text2" style="border:1px solid #D3C8AA;"><?php echo $data['CmsMenu']['target']?></td>
            <td style="border:1px solid #D3C8AA;">
                <div class="line2">
                    <a href="<?php echo $settings['cms_url']?>Menu/Add/<?php echo $data['CmsMenu']['id']?>" class="nav_6">edit</a>
                    <a href="javascript:void(0)" class="nav_6" onclick="DeleteItem('<?php echo $data['CmsMenu']['id']?>')">delete</a>
                </div>
            </td>
        </tr>
        <?php endforeach;?>
