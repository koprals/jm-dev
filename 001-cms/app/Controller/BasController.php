<?php
class BasController extends AppController
{
	var $ControllerName		=	"Bas";
	var $ModelName			=	"User";
	...
	function beforeFilter()
	{
		...
		$find					=	$this->MyAco->find("first",array(
										"conditions"	=>	array(
											"LOWER(MyAco.alias)"	=>	strtolower("BA")
										)
									));
	...
	function ListItem()
	{
		...
		if(!empty($this->profile['City']['id'])) {
			$filter_paginate	=	array(
				"{$this->ModelName}.city_id" => $this->profile['City']['id'],
				"{$this->ModelName}.user_type_id"	=>	"1"
			);
		} else {
			$filter_paginate	=	array(
				"{$this->ModelName}.user_type_id"	=>	"1"
	...
	function Add()
	{
		...
		if(!empty($this->request->data))
		{
			...
			{
				$this->request->data[$this->ModelName]['user_type_id']	=	"1";//BA = 1,TL = 2 ;
	...
	function Edit($ID=NULL,$page=1,$viewpage=50)
	{
		...
		*/
		...
		else
		{
			...
			$this->{$this->ModelName}->ValidateEditBA();
	...
	function tempAddSchedule() {
		$bas = $this->User->find('all', array(
			'conditions' => array(
				'User.user_type_id' => 1
			)
		));
		...
		foreach($bas as $key => $value){
			$this->loadModel('ScheduleBa');
			$saveData['ScheduleBa'] = array(
				'schedule_id' => '1586',
				'ba_id' => $value['User']['id'],
				'status' => 1
			);

			$this->ScheduleBa->create();
			$this->ScheduleBa->set($saveData);
			$test = $this->ScheduleBa->save($saveData);
			var_dump($test);
