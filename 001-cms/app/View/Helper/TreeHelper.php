    ...
    function generate ($data, $settings = array (),$current_selected=array()) {
        ...
        foreach ($data as $i => $result) {
            ...
            if ($__addType) {
                ...
                if ($type) {
                    ...
                    if($count==1)
                    {
                    	if($admin_review==0)
						{
							$return .= '<' . $type .  $typeAttributes . ' class="filetree" id="browser" ><li><span class="add_category" onclick="location.href=\''.Configure::read('webUrl').'Product/AddCategories/'.$result[$model]['parent_id'].'\'">Add New Category</span></li>';
						}
						else
						{
            ...
            if ($hasVisibleChildren) {
                ...
                if ($itemType && $admin_review==0) {
                    $return .= '<span class="add_category" style="margin-left:15px" onclick="location.href=\''.Configure::read('webUrl').'Product/AddCategories/'.$result[$model]['id'].'\'">Add Sub Category '.$result[$model][$alias].'</span></' . $itemType . '>';
                }
                $return .= $this->__suffix();
            }
        }
