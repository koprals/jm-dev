...
<?php $this->Paginator->options(array(
				'url'	=> array(
					...
					'action'		=> 'NotSoldProductListItem/limit:'.$viewpage,
				),
...
function ChangeStatus(msg,id,status)
{
	...
	if(a)
	{
		$.getJSON("<?php echo $settings["cms_url"].$ControllerName?>/ChangeStatus/"+id+"/"+status,function(result){
			...
			{
				$("#contents_area").load("<?php echo $settings["cms_url"].$ControllerName?>/NotSoldProductListItem/page:<?php echo $page?>/limit:<?php echo $viewpage.$ordered?>",function(){
			...
			if( $("#action").val() == "delete" )
			{
				...
				if(a)
				{
					...
					$.getJSON("<?php echo $settings["cms_url"].$ControllerName?>/DeleteMultiple/",{
						...
						if(result.data.status == "1")
						{
							$("#contents_area").load("<?php echo $settings["cms_url"].$ControllerName?>/NotSoldProductListItem/page:1/limit:<?php echo $viewpage.$ordered?>",function(result){
								$("#view, input:checkbox, #action").uniform();
								$('.tipS').tipsy({gravity: 's',fade: true});
								$("a[rel^='lightbox']").prettyPhoto({
									social_tools :''
    ...
    <div class="itemsPerPage">
      ...
      <div class="dataTables_length">
				<label>
					...
					<?PHP echo $this->Form->select("view",array(1=>1,5=>5,10=>10,20=>20,50=>50,100=>100,200=>200,1000=>1000),array("onchange"=>"onClickPage('".$settings["cms_url"].$ControllerName."/NotSoldProductListItem/limit:'+this.value+'".$ordered."/','#contents_area')","empty"=>false,"default"=>$viewpage))?>
				</label>
			</div>
		</div>
	</div>
