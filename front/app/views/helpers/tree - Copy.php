 ...
 */
    ...
    function generate ($data, $settings = array ()) {
        ...
        if ($depth == 0) {
			...
			//$this->addItemAttribute('class', "expand");
        ...
        foreach ($data as $i => $result) {
            ...
            if ($element) {
				...
				$url	=	empty($result[$model]['url']) ? $link.$result[$model]['id'] : $valid_url;
				$target	=	$result[$model]['target'];
				
				if($firstChild && $result[$model]['parent_id']!="98")
				{
					$seeall	=	empty($result[$model]['url']) ? $link.$result[$model]['parent_id'] : $valid_url;
					$content = "<a href='".$seeall."' target='".$target."' style='font-weight:bold'>See All</a>"."<a href='".$url."' target='".$target."' >".$result[$model][$alias]."</a>";
				...
				else
				{
					if($hasChildren)
					{
						$content = "<a href='".$url."' target='".$target."' >".$result[$model][$alias]."</a>";
					}
					else
					{
						$content = "<a href='".$url."' target='".$target."' >".$result[$model][$alias]."</a>";
            ...
            if ($__addType) {
                ...
                if ($type) {
                    ...
                    $return .= '<' . $type .  $typeAttributes . ' class="acitem" >';
                }
            }
            if ($indent) {
                $return .= "\r\n" . $whiteSpace . "\t";
