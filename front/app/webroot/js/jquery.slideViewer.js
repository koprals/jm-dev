...
jQuery.fn.slideViewerPro = function(settings) {
	...
	return this.each(function(){
		...
		*/
		...
		var r_enabled = true;
		var l_enabled = true;
		var margin_bottom	=	($.browser.msie) ? 10 : 5 ;
    container.find("ul").css("width" , slideViewerWidth)
    .wrap(jQuery("<div style='width:580px; margin:17px auto "+margin_bottom+"px auto;overflow: hidden; position: relative; top: 0; left: 0;border:0px solid black;'>"));
		...
		container.each(function(i) {
			...
			jQuery("div#thumbSlider" + j).css({position: "relative", left: contBorder-3, top: settings.thumbsTopMargin+"px", width: settings.thumbs*thumbsWidth+((settings.thumbsRightMargin*settings.thumbs)-settings.thumbsRightMargin), height: thumbsHeight, textAlign: "center", overflow: "hidden", margin: "0 auto"});
			...
			jQuery("a#left" + j).css({display: "block", textAlign: "center", width: settings.buttonsWidth + "px" , height: thumbsHeight+"px",  margin: -(thumbsHeight-settings.thumbsTopMargin) +"px 0 0 -5px", textDecoration: "none", lineHeight: thumbsHeight+"px", color: settings.buttonsTextColor});
			...
			jQuery("div#thumbSlider" + j + ", a#left" + j + ", a#right" + j).wrapAll("<div style='width:100%; display: "+uiDisplay+"; float:left;border:0px solid black;' id='ui"+j+"'><\/div>");			
